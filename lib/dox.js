...
exports.contextPatternMatchers = [
  ...
  function (str) {
    // anonymous function expression exported as a default
    if (/^\s*export\s+default\s+async function\s*\(/.exec(str)) {
      return {
          type: 'function'
        , name: RegExp.$1 // undefined
        , string: RegExp.$1 + '()'
  ...
  function (str) {
    // function expression
    if (/^return\s+async function(?:\s+([\w$]+))?\s*\(/.exec(str)) {
      return {
          type: 'function'
        , name: RegExp.$1
        , string: RegExp.$1 + '()'
  ...
  function (str) {
    ...
    if (/^\s*(?:const|var|let)\s+([\w$]+)\s*=\s*(?:async)?\s*(\(\)|[a-zA-Z]\w*|\((?:\.{3})?[a-zA-Z]\w*\s*(,\s*(?:\.{3})?[a-zA-Z]\w*)*\))\s*=>/.exec(str)) {
      return {
          type: 'function'
        , name: RegExp.$1
        , string: RegExp.$1 + '()'
  ...
  function (str, parentContext) {
    ...
    if (/^\s*([\w$.]+)\s*\.\s*prototype\s*\.\s*([\w$]+)\s*=\s*(?:async)?\s*(\(\)|[a-zA-Z]\w*|\((?:\.{3})?[a-zA-Z]\w*\s*(,\s*(?:\.{3})?[a-zA-Z]\w*)*\))\s*=>/.exec(str)) {
      return {
          type: 'method'
        , constructor: RegExp.$1
        , cons: RegExp.$1
