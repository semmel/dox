...
exports.contextPatternMatchers = [
  ...
  function (str) {
    if (/^\s*(export(\s+default)?\s+)?async function\s+([\w$]+)\s*\(/.exec(str)) {
      return {
          type: 'function'
        , name: RegExp.$3
        , string: RegExp.$3 + '()'
  ...
  function (str) {
    ...
    if (/^\s*export\s+default\s+(?:async)?\s*function\s*\(/.exec(str)) {
      return {
          type: 'function'
  ...
  function (str) {
    // function expression
    if (/^return\s+(?:async)?\s*function(?:\s+([\w$]+))?\s*\(/.exec(str)) {
      return {
          type: 'function'
        , name: RegExp.$1
        , string: RegExp.$1 + '()'
  ...
  function (str) {
    // function expression
    if (/^\s*(?:const|let|var)\s+([\w$]+)\s*=\s*(?:async)?\s*function/.exec(str)) {
      return {
          type: 'function'
        , name: RegExp.$1
        , string: RegExp.$1 + '()'
  ...
  function (str) {
    ...
    if (/^\s*(?:const|var|let)\s+([\w$]+)\s*=\s*(?:async)?\s*(\(\)|[a-zA-Z_$]\w*|\((?:\.{3})?[a-zA-Z_$]\w*\s*(,\s*(?:\.{3})?[a-zA-Z_$]\w*)*\))\s*=>/.exec(str)) {
      return {
          type: 'function'
        , name: RegExp.$1
        , string: RegExp.$1 + '()'
  ...
  function (str, parentContext) {
    // prototype method
    if (/^\s*([\w$.]+)\s*\.\s*prototype\s*\.\s*([\w$]+)\s*=\s*(?:async)?\s*function/.exec(str)) {
      return {
          type: 'method'
  ...
  function (str, parentContext) {
    // prototype arrow method
    if (/^\s*([\w$.]+)\s*\.\s*prototype\s*\.\s*([\w$]+)\s*=\s*(?:async)?\s*(\(\)|[a-zA-Z_$]\w*|\((?:\.{3})?[a-zA-Z_$]\w*\s*(,\s*(?:\.{3})?[a-zA-Z_$]\w*)*\))\s*=>/.exec(str)) {
      return {
          type: 'method'
        , constructor: RegExp.$1
        , cons: RegExp.$1
  ...
  function (str, parentContext) {
    // inline method
    if (/^\s*([\w$.]+)\s*:\s*async function/.exec(str)) {
      return {
        type: 'method'
        , constructor: parentContext.name
        , cons: parentContext.name
  ...
  function (str) {
    // method
    if (/^\s*([\w$.]+)\s*\.\s*([\w$]+)\s*=\s*async function/.exec(str)) {
      return {
          type: 'method'
        , receiver: RegExp.$1
        , name: RegExp.$2
